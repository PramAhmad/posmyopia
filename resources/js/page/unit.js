$(function(){
    const dtTable = window.LaravelDataTables["unit-table"]
    ...
    $('table.dataTable tbody').on('click', 'tr td .btn-edit', function(){
        ...
        $.ajax({
            type: 'POST',
            url: `${baseUrl}/unit/${t.data('id')}/edit`,
            data: {id:t.data('id')},
            beforeSend: function() {
    ...
    $('form').on('submit', function(e){
        ...
        const actionUrl = id ? `${baseUrl}/unit/${id}/edit` : `${baseUrl}/unit`
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function() {
                saveBtn.html('<div class="spinner-border spinner-border-sm" unit="status"></div>').prop('disabled', true)
