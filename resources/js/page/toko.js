$(function(){
    const dtTable = window.LaravelDataTables["toko-table"]

    $('.btn-add-toko').on('click', function(){
        $('.modal-title').text('Add toko')
    })
    ...
    $('table.dataTable tbody').on('click', 'tr td .btn-edit', function(){
        ...
        $.ajax({
            type: 'POST',
            url: `${baseUrl}/toko/${t.data('id')}/edit`,
            data: {id:t.data('id')},
            beforeSend: function() {
            ...
            success: function(response) {
                ...
                $('#modal-toko').modal('show');
            
                $.each(response.data, function(i, item) {
                    console.log(i)
                    if (i === 'logo') {
                        const fileUrl = `/upload/toko/${item}`;            
                        $('#logo-preview').attr('src', fileUrl).show();
                        $('#logo-filename').text(item).show();            
                        $('.modal form').find('input[name=logo]').val('');
                    }   else{
                        $('.modal form').find(`input[name=${i}]`).val(item);
                    }   
                    $('.modal-content').find(`.btn-save`).attr('data-id', response.data.id);
                });
            },
    ...
    $('form').on('submit', function(e){
        ...
        const actionUrl = id ? `${baseUrl}/toko/${id}/edit` : `${baseUrl}/toko`
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function() {
                saveBtn.html('<div class="spinner-border spinner-border-sm" toko="status"></div>').prop('disabled', true)
