$(function(){
    const dtTable = window.LaravelDataTables["category-table"]
    ...
    $('table.dataTable tbody').on('click', 'tr td .btn-edit', function(){
        ...
        $.ajax({
            type: 'POST',
            url: `${baseUrl}/category/${t.data('id')}/edit`,
            data: {id:t.data('id')},
            beforeSend: function() {
    ...
    $('form').on('submit', function(e){
        ...
        const actionUrl = id ? `${baseUrl}/category/${id}/edit` : `${baseUrl}/category`
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function() {
                saveBtn.html('<div class="spinner-border spinner-border-sm" category="status"></div>').prop('disabled', true)
