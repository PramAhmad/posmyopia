$(function(){
    const dtTable = window.LaravelDataTables["product-table"]
    ...
    $('table.dataTable tbody').on('click', 'tr td .btn-edit', function(){
        ...
        $.ajax({
            type: 'POST',
            url: `${baseUrl}/product/${t.data('id')}/edit`,
            data: {id:t.data('id')},
            beforeSend: function() {
            ...
            success: function(response) {
                ...
                $.each(response.data, function(i, item) {
                    console.log(i)
                    if (i === 'image') {
                        const fileUrl = `/upload/product/${item}`;            
                        $('#image-preview').attr('src', fileUrl).show();
                        $('#image-filename').text(item).show();            
                        $('.modal form').find('input[name=image]').val('');
                    }   else{
    ...
    $('form').on('submit', function(e){
        ...
        const actionUrl = id ? `${baseUrl}/product/${id}/edit` : `${baseUrl}/product`
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function() {
                saveBtn.html('<div class="spinner-border spinner-border-sm" product="status"></div>').prop('disabled', true)
    ...
    $('input[name=image]').on('change', function(){
        ...
        reader.onload = function(e) {
            ...
            $('#image-filename').text(file.name).show();
        }
        reader.readAsDataURL(file);
    })
    // toko_id change post ajax
