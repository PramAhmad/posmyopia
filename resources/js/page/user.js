$(function(){
    const dtTable = window.LaravelDataTables["user-table"]
    ...
    $('table.dataTable tbody').on('click', 'tr td .btn-edit', function(){
        ...
        $.ajax({
            type: 'POST',
            url: `${baseUrl}/user/${t.data('id')}/edit`,
            data: {id:t.data('id')},
            beforeSend: function() {
                t.html('<div class="spinner-border spinner-border-sm" user="status"></div>').prop('disabled', true)
            },
            success: function(response) {
                $('.modal-title').text('Edit User')
                $('#modal-user').modal('show')
                $('.modal form').find('input[type=password]').removeAttr('required')

                $.each(response.data, function(i, item) {
                    $('.modal form').find(`input[name=${i}],select[name="${i}"],textarea[name="${i}"`).val(item)
    ...
    $('form').on('submit', function(e){
        ...
        const actionUrl = id ? `${baseUrl}/user/${id}/edit` : `${baseUrl}/user`
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function() {
                saveBtn.html('<div class="spinner-border spinner-border-sm" user="status"></div>').prop('disabled', true)
    ...
    $('.modal').on('hidden.bs.modal', function () {
        ...
        $('.modal form').find('input[type=password]').attr('required', true)
    })
